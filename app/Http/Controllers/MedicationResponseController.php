...
class MedicationResponseController extends Controller
{
    ...
    foreach($nafdact_products as $product) {
        $sample_payload = self::get_sample_payload();
        ...
        // id
        $sample_payload['id'] = $this->change_type_and_lower_case(Str::random(25) . '_' . data_get($product, 'product_name'));
        ...
        // code
        $sample_payload['code'] = [
            'coding' => $this->get_all_type_of_codes($product, $dm_and_d_detail),
        ];
        ...
        $sample_payload['doseForm'] = [
            'coding' => $this->get_dose_form($dm_and_d_detail),
        ];

        // ingredient
        $sample_payload['ingredient'] = $this->get_all_ingredients_data($product, $dm_and_d_detail);

        // definition
        $sample_payload['definition'] = [
            'reference' => 'MedicationKnowledge/medicationknowledge-'.$this->change_type_and_lower_case(data_get($product, 'product_name'), '-'),
        ];
        ...
        foreach($dm_and_d_detail as $detail) {
            ...
            if($ingredients) {
                foreach($ingredients as $ingredient) {
                    ...
                    $data['item'] = [
                        'concept' => (object) [
                            'coding' => $this->get_ingredient_codings($detail, $ingredient),
                        ]
                    ];
                    $data['strengthRatio'] = [
